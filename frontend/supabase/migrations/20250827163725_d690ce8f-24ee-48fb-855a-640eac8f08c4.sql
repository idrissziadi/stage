-- Restructurer les tables inscription et offre - Gérer les dépendances

-- 1. Supprimer les politiques qui dépendent de id_stagiaire
DROP POLICY IF EXISTS "Stagiaires can view Cours for their enrolled modules" ON public.cours;
DROP POLICY IF EXISTS "Stagiaires can view their own Inscription" ON public.inscription;
DROP POLICY IF EXISTS "Admin can manage Inscriptions" ON public.inscription;

-- 2. Renommer la table inscription vers offre
ALTER TABLE public.inscription RENAME TO offre;

-- 3. Supprimer la colonne id_stagiaire de la table offre
ALTER TABLE public.offre DROP COLUMN id_stagiaire;

-- 4. Ajouter une nouvelle colonne id_offre comme clé primaire auto-incrémentée
...
-- 9. Recréer la politique pour les cours avec la nouvelle logique
CREATE POLICY "Stagiaires can view Cours for their enrolled modules" 
ON public.cours 
FOR SELECT 
USING (
  EXISTS (
    SELECT 1 
    FROM inscription i
    JOIN offre o ON i.id_offre = o.id_offre
    JOIN specialite s ON o.id_specialite = s.id_specialite
    JOIN module m ON m.id_specialite = s.id_specialite
    WHERE i.id_stagiaire = (
      SELECT stagiaire.id_stagiaire 
      FROM stagiaire 
      WHERE stagiaire.user_id = auth.uid()
    ) 
    AND m.id_module = cours.id_module
  )
);

-- 10. Créer des triggers pour mettre à jour updated_at
