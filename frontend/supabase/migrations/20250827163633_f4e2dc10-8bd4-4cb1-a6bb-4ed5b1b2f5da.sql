-- Restructurer les tables inscription et offre

-- 1. Renommer la table inscription vers offre
ALTER TABLE public.inscription RENAME TO offre;

-- 2. Renommer la colonne id_stagiaire vers id_offre dans la nouvelle table offre
ALTER TABLE public.offre RENAME COLUMN id_stagiaire TO id_offre;

-- 3. Modifier le type de id_offre pour être un entier auto-incrémenté
ALTER TABLE public.offre ALTER COLUMN id_offre SET DATA TYPE integer;
ALTER TABLE public.offre ALTER COLUMN id_offre SET DEFAULT nextval('offre_id_offre_seq'::regclass);

-- Créer une séquence pour id_offre si elle n'existe pas
CREATE SEQUENCE IF NOT EXISTS offre_id_offre_seq OWNED BY public.offre.id_offre;
