...
const sequelize = new Sequelize('stage_db', 'root', '', {
  host: 'localhost',
  dialect: 'mysql',
  logging: false
});

async function testDatabaseEtablissementRegionale() {
  console.log('🧪 Test de la base de données pour EtablissementRegionale\n');
  ...
  try {
    ...
    const [userIFEP] = await sequelize.query(`
      SELECT 
        c.id_compte,
        c.username,
        c.role,
...
if (require.main === module) {
  ...
  testDatabaseEtablissementRegionale();
}

module.exports = { testDatabaseEtablissementRegionale };
